...
@mcp.tool()
def run_excel_code(code: str, file_path: str, sheet_name: str = None,
                  skiprows: int = None, header = None, usecols = None,
                  encoding: str = None, auto_detect: bool = True) -> dict:
    """增强版Excel代码执行，支持复杂格式参数、智能检测和编码处理。
    
    Args:
        code: 要执行的数据处理代码字符串
        file_path: Excel文件路径
        sheet_name: 可选，工作表名称。如果为None，加载第一个工作表
        skiprows: 可选，跳过的行数，用于处理多级列头
